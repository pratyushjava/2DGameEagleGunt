...
void Viewport::draw() const {
  //IOmod::getInstance().writeText("Tracking "+objectToTrack->getName(), (viewWidth/2)+(viewWidth/4), 30);
  //IOmod::getInstance().writeText(gdata.getXmlStr("screenTitle/title"), (viewWidth/2) - (viewWidth / 8), 30);
  IOmod::getInstance().writeText(gdata.getXmlStr("screenTitle/title"), 30, viewHeight-30);
}
